 ...
 *  - `setor`: rotaciona dentro de um setor (round-robin, online primeiro).
 *  - `atendente_fixo`: atribui sempre a pessoa específica.
 * Saídas: `atribuido` (achou/atribuiu alguém) e `sem_atendente` (sem opção).
 */
export interface ConfigDistribuirAtendimento {
  /** "setor" = rotação dentro do setor (default). "atendente_fixo" = pessoa fixa. */
  modo?: "setor" | "atendente_fixo";
  /** Setor (tabela `setores`) — usado quando modo="setor". */
  setorId?: number;
...
export const TIPO_PASSO_META: ReadonlyArray<TipoPassoMeta> = [
  ...
  { id: "ia_responder", label: "Responder com IA", descricao: "Gera UMA resposta com IA e devolve o controle pro fluxo (não conduz a conversa nem age sozinho). Use quando VOCÊ decide o próximo passo depois (ex: Condição). Pra um bot que conversa E agenda sozinho, prefira o Atendente IA.", cor: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300", grupo: "ia" },
  ...
  { id: "ia_atendente", label: "Atendente IA", descricao: "O 'tudo em um': conduz a conversa inteira (qualifica, explica, tira dúvidas), espera o cliente sozinho e dispara ações (agendar, transferir...) na hora certa. Busca horários reais antes de marcar. Use pra um bot que conduz o atendimento de ponta a ponta. O roteiro vai no prompt.", cor: "bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300", grupo: "ia" },
