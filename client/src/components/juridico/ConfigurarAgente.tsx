  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Library className="h-5 w-5 text-info-fg" /> Configurar o Agente Jurídico</DialogTitle>
        ...
        <div className="space-y-1.5">
          ...
          {(minhasFontesQ.data ?? []).map((f: any) => (
            <div key={f.id} className="flex items-start justify-between gap-2 border rounded-md p-2">
              ...
              <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 hover:text-danger-fg" disabled={delFonteMut.isPending} onClick={() => delFonteMut.mutate({ id: f.id })}><Trash2 className="h-3.5 w-3.5" /></Button>
            </div>
          ))}
