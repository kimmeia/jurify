...
function IntegrationLogo({ id, className }: { id: string; className?: string }) {
  if (id === "sentry") {
    return (
      <div className={`flex items-center justify-center rounded-lg bg-info/10 ${className}`}>
        <AlertCircle className="h-4 w-4 text-info-fg" />
      </div>
    );
  }
  if (id === "whatsapp_cloud") {
    return (
      <div className={`flex items-center justify-center rounded-lg bg-success/10 ${className}`}>
  ...
  return (
    <div className={`flex items-center justify-center rounded-lg bg-success/10 ${className}`}>
      <span className="text-base font-bold text-success-fg">J</span>
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  if (status === "conectado") {
    return (
      <Badge className="bg-success/15 text-success-fg border-success/30 hover:bg-success/15">
        ...
        {isErro && (
          <div className="space-y-3 pt-1">
            ...
            {showEditForm && integracao.id !== "whatsapp_cloud" && integracao.id !== "sentry" && (
              <div className="space-y-1.5 rounded-lg border border-warning/30 bg-warning-bg/50 p-2.5">
                <Label className="text-[10px] font-semibold text-warning-fg">
                  Nova API key
                </Label>
