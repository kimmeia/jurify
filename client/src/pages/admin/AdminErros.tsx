      ...
      {data?.capturaConfigurada === false && (
        <div className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning-bg/30 px-4 py-3 text-sm">
          ...
          <div className="min-w-0 text-muted-foreground">
            ...
            O backend só manda erros pro Sentry quando a variável <code>SENTRY_DSN_BACKEND</code> está
            definida no Railway — e o servidor subiu sem ela. Pra confirmar: Railway → serviço do app →
            Variables → <code>SENTRY_DSN_BACKEND</code> com a DSN do projeto, depois redeploy. Até lá,
            "zero erros" aqui não prova nada.
          </div>
        </div>
      )}
