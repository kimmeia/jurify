...
export function ConversationDiff({ conversaId }: { conversaId: number }) {
  ...
  return (
    <div className="mx-4 mt-2 rounded-xl border border-warning/30 bg-gradient-to-r from-warning-bg/80 to-warning-bg/40 overflow-hidden">
      <div className="flex items-stretch">
        <button
          onClick={() => setExpandido((v) => !v)}
          className="flex-1 min-w-0 px-3.5 py-2 flex items-center gap-2.5 hover:bg-warning-bg/40 transition-colors text-left"
        >
          <div className="w-6 h-6 rounded-lg bg-warning flex items-center justify-center flex-shrink-0">
            <Zap className="h-3 w-3 text-white" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-[10px] font-bold text-warning-fg uppercase tracking-wide">
              Desde sua última resposta
              {diasDesde > 0 && <span className="text-warning-fg">· há {diasDesde}d</span>}
            </div>
          ...
          <ArrowUpRight
            className={"h-3.5 w-3.5 text-warning-fg transition-transform " + (expandido ? "rotate-90" : "")}
          />
        </button>
        <button
          onClick={dispensar}
          ...
          className="px-2.5 flex items-center justify-center text-warning-fg/70 hover:text-warning-fg hover:bg-warning-bg/60 transition-colors"
        >
      ...
      {expandido && (
        <div className="border-t border-warning/30 px-3.5 py-2.5 bg-white/60 space-y-2">
          {atos.length > 0 && (
            <div>
              <p className="text-[10px] font-bold text-warning-fg uppercase mb-1">Atos processuais</p>
          ...
          {prazos.length > 0 && (
            <div>
              <p className="text-[10px] font-bold text-danger-fg uppercase mb-1">Prazos próximos (48h)</p>
              <div className="space-y-1">
                {prazos.map((p: any) => (
                  <div key={p.id} className="text-[11px] text-danger-fg">
