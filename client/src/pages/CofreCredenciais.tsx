 ...
 *
 * Acesso: usuário logado vê só as suas credenciais. Backend filtra por criadoPor=user.id.
 *
 * Fluxo:
...
export default function CofreCredenciais() {
  ...
  const listaQuery = (trpc as any).cofreCredenciais.listarMinhas.useQuery();
  const removerMut = (trpc as any).cofreCredenciais.removerMinha.useMutation({
    onSuccess: () => {
  ...
  const validarMut = (trpc as any).cofreCredenciais.validarMinha.useMutation({
    onSuccess: (data: any) => {
...
function NovaCredencialDialog({ onCriado }: { onCriado: () => void }) {
  ...
  const criarMut = (trpc as any).cofreCredenciais.cadastrarMinha.useMutation({
    onSuccess: () => {
