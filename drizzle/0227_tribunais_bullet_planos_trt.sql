-- Justiça do Trabalho entrou na cobertura por consulta pública (TRT2 e
-- TRT15): o bullet "Vigia…" dos 3 planos dizia "TJSP e TRTs ainda não" e
-- passou a mentir. O texto novo é o que `bulletVigiaPlano` (shared) gera.
--
...
UPDATE planos
  SET features = JSON_REPLACE(features, '$[3]',
    'Vigia 300 processos nos tribunais cobertos (12 TJs e 4 TRFs, mais TRF5, TRT2 e TRT15 por consulta pública — TJSP ainda não; Justiça do Trabalho: TRT2 e TRT15 por consulta pública) · 15 CPFs/CNPJs (novas ações: comprovado no TJCE)')
  WHERE slug = 'atende'
...
UPDATE planos
  SET features = JSON_REPLACE(features, '$[5]',
    'Vigia 1.000 processos nos tribunais cobertos (12 TJs e 4 TRFs, mais TRF5, TRT2 e TRT15 por consulta pública — TJSP ainda não; Justiça do Trabalho: TRT2 e TRT15 por consulta pública) · 50 CPFs/CNPJs (novas ações: comprovado no TJCE)')
...
UPDATE planos
  SET features = JSON_REPLACE(features, '$[4]',
    'Vigia 2.500 processos nos tribunais cobertos (12 TJs e 4 TRFs, mais TRF5, TRT2 e TRT15 por consulta pública — TJSP ainda não; Justiça do Trabalho: TRT2 e TRT15 por consulta pública) · 150 CPFs/CNPJs (novas ações: comprovado no TJCE)')
  WHERE slug = 'escala'
