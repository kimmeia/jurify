...
export async function seedTestEscritorio(
  ...
  // `maxColaboradores` estava aqui e não existe na tabela — o Drizzle ignora
  // chave desconhecida em silêncio, e o erro de tipo nunca apareceu porque
  // `tests/` não está no `include` do tsconfig. Só saiu do escuro quando este
  // arquivo mudou de casa pra `server/`. O limite de colaboradores não é do
  // escritório e sim do plano; nada aqui dependia dele.
