...
export const clientesRouter = router({
  ...
  detalhe: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ ctx, input }) => {
    ...
    // Nome de quem encerrou/cancelou o serviço (auditoria no cabeçalho).
    let servicoEncerradoPorNome: string | null = null;
    if (c.servicoEncerradoPor) {
      const [colab] = await db.select({ nome: users.name }).from(colaboradores).innerJoin(users, eq(users.id, colaboradores.userId)).where(eq(colaboradores.id, c.servicoEncerradoPor)).limit(1);
      servicoEncerradoPorNome = colab?.nome ?? null;
    }
