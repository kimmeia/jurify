...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // Add asaasPaymentId column if table already existed
      try { await connection.query(`ALTER TABLE kanban_cards ADD COLUMN asaasPaymentIdKCard VARCHAR(64) NULL`); } catch { /* already exists */ }
    } catch (err: any) {
