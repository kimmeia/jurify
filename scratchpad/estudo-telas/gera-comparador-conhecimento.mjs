/**
 * Comparador ANTES | DEPOIS da fusão Base Jurídica + JurisIA.
 *
 * Cópia do `gera-comparador.mjs` com os achados deste assunto. Aqui as
 * mudanças são blocos que NASCEM, não larguras que vazam — por isso o anel
 * fica num elemento que existe nos DOIS lados e o texto manda olhar o que
 * apareceu em volta dele. Sem isso o anel sairia só de um lado e a
 * comparação viraria um lado vazio.
 *
...
const TELAS = [
  ["jurisia", "JurisIA · o advogado"],
  ["admin-biblioteca", "Painel · Conhecimento jurídico"],
  ["admin-acervo", "Painel · o acervo por dentro"],
];
...
const ITENS = [
  {
    id: "resposta-ementa",
    olhe: "O anel cerca a resposta inteira nos dois lados. No antes ela tem duas partes: o texto e o painel de números. No depois nasce uma terceira, entre os dois — o bloco azul com três ementas, cada uma com órgão, data e “ver no tribunal”.",
    tela: "jurisia", vista: "desktop",
    curto: "A resposta passou a citar ementa",
    alvo: { texto: "Como este tribunal vem decidindo", subir: 3 },
    pontos: [[
      "Antes não havia uma linha de decisão pra citar",
      "O acervo de hoje vem do DataJud, que entrega classe, vara e movimento — e nenhuma ementa. Dava pra dizer como os processos terminam; não dava pra fundamentar a peça. Agora a ementa vem primeiro, e o número continua embaixo, com a frase que separa os dois.",
    ], [
      "O link não é enfeite",
      "É o que separa citação de invenção: o advogado abre o acórdão no site do tribunal antes de assinar. Sem endereço oficial, a frase não entra.",
    ]],
  },
  {
    id: "coluna-fontes",
    olhe: "O anel está na coluna “O que a IA está usando”. No antes ela tem UM cartão (Acervo público). No depois tem três, e a ordem importa: Jurisprudência em cima, porque é a que vira citação.",
    tela: "jurisia", vista: "desktop",
    curto: "A coluna diz as três fontes, não uma",
    alvo: { texto: "O que a IA está usando", subir: 1 },
    pontos: [[
      "Um cartão escondia duas metades",
      "A conversa já lia os autos do cliente e a biblioteca da casa no mesmo turno — mas a tela só falava do acervo público. Quem olhava não sabia de onde tinha saído metade da resposta.",
    ]],
  },
  {
    id: "abas-viram-uma",
    olhe: "O anel está na régua de abas. Conte os nomes dentro dele: no antes são três (Agentes IA · Base Jurídica · JurisIA); no depois são dois (Agentes IA · Conhecimento jurídico).",
    tela: "admin-biblioteca", vista: "desktop",
    curto: "Duas abas viraram uma",
    alvo: { texto: "Agentes IA", subir: 1 },
    pontos: [[
      "Eram telas separadas por data de nascimento, não por assunto",
      "As duas alimentam a MESMA resposta ao advogado. Separadas, ninguém via o buraco do meio: o acervo não tem texto de decisão, e a biblioteca só cresce quando alguém sobe arquivo à mão.",
    ]],
  },
  {
    id: "fontes-oficiais",
    olhe: "O anel está na lista de fontes da biblioteca, que existe nos dois lados. Olhe o que passou a existir ACIMA dela: a tabela das fontes oficiais (com EMENTA × SÓ NÚMERO, de quanto em quanto tempo o robô volta e o estado de cada uma) e os cartões de entendimento por tribunal.",
    tela: "admin-biblioteca", vista: "desktop",
    curto: "De onde vem o material, e o que cada tribunal decide",
    alvo: { texto: "Fontes da base", subir: 1 },
    pontos: [[
      "O robô volta sozinho, e a tela diz quando",
      "Cada fonte tem a sua frequência e o seu estado — em dia, coletando, bloqueada. Fonte bloqueada aparece em vermelho com a data da última tentativa, em vez de sumir em silêncio.",
    ], [
      "Entendimento por região é o que muda a peça",
      "A média nacional não ajuda a escolher a tese. O cartão por tribunal diz quantas decisões existem, quanto termina a favor e quantas são acórdão — o que dá pra citar.",
    ]],
  },
];
...
const PENDENTES = [
  ["Ligar a coleta de ementa depende de uma sondagem que só você pode rodar",
   "O botão “Sondar” já existe em produção (Painel → IA → JurisIA → ferramentas técnicas) e nunca foi rodado. Ele diz quais tribunais respondem DO NOSSO SERVIDOR e quais devolvem ementa. Os números de ementa desta tela são exemplo até essa sondagem rodar."],
  ["Busca precisa pede índice de texto, não só semelhança",
   "Hoje a biblioteca carrega todas as fontes na memória e compara uma a uma. Serve pra centenas de súmulas; com dezenas de milhares de ementas não fica de pé. O conserto é somar busca por texto à busca por significado."],
  ["Credencial de tribunal NÃO entra aqui, de propósito",
   "Credencial só abre o que é seu, e base de jurisprudência é feita de processo de terceiro. Fora isso, varrer o tribunal em volume com login de advogado arrisca o acesso da OAB de vocês. Acórdão publicado é aberto — é por essa porta que o robô entra."],
  ["O nome da tela",
   "Chamei de “Conhecimento jurídico”. Se preferir outro nome, é trocar uma palavra."],
  ["Cobrança não muda com a fusão",
   "Juntar as abas é organização de tela; o add-on continua sendo vendido igual. Se a coleta crescer o custo, isso é conversa de preço — separada desta."],
];
...
<title>JuridFlow — Conhecimento jurídico · JuridFlow — antes e depois, lado a lado</title>
<style>
${fontes}
:root{
