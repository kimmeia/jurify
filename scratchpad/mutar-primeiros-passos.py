...
MUTACOES = [
    ...
    ("monitor de CPF (novas ações) conta como processo vigiado", ROUTER,
     '        eq(motorMonitoramentos.tipoMonitoramento, "movimentacoes"),\n', ""),
    ("lead criado sozinho pelo WhatsApp conta como cliente cadastrado", ROUTER,
     '        ne(contatos.origem, "whatsapp"),\n', ""),
    # ── de quem é o escritório / qual registro ──
    ("escritório resolvido pra OUTRO usuário (não o da sessão)", ROUTER,
     "const vinculo = await getEscritorioPorUsuario(ctx.user.id);",
     "const vinculo = await getEscritorioPorUsuario(1);"),
    ("'feito em' vira a data do registro mais NOVO (desc em todos)", ROUTER,
     "import { and, asc, eq,", "import { and, desc as asc, eq,"),
    ("consulta de contatos sem ORDER BY", ROUTER,
    ...
    ("resumo da Central some quando completo", COMP,
     "  const completo = feitos === total;\n",
     "  const completo = feitos === total;\n  if (completo) return null;\n"),
    ("resumo completo sem o selo 'tudo feito'", COMP,
     "        {completo && (\n          <span className=\"ml-auto inline-flex items-center gap-1 text-apoio font-bold text-success-fg\">\n            <CheckCircle2 className=\"h-3.5 w-3.5 shrink-0\" />\n            tudo feito\n          </span>\n        )}\n",
     ""),
    ("'ver no Dashboard' fica mesmo com tudo feito (bloco já sumiu de lá)", COMP,
     "const dashboardMontaOBloco = !completo && !pacoteProcessualPuro(contratados);",
     "const dashboardMontaOBloco = !pacoteProcessualPuro(contratados);"),
    ("'ver no Dashboard' fica no contrato só-processos (Dashboard não monta o bloco)", COMP,
     "const dashboardMontaOBloco = !completo && !pacoteProcessualPuro(contratados);",
     "const dashboardMontaOBloco = !completo;"),
    ("link incondicional (fora do {dashboardMontaOBloco && ...})", COMP,
    ...
    ("Vigiar volta a abrir o diálogo de CPF (novas ações)", SHARED,
     '    rota: "/processos?tab=movimentacoes&abrirMonitor=1",',
     '    rota: "/processos?tab=novas-acoes&novo=1",'),
    ("Vigiar na aba certa mas sem abrir o diálogo", SHARED,
     '    rota: "/processos?tab=movimentacoes&abrirMonitor=1",',
     '    rota: "/processos?tab=movimentacoes&novo=1",'),
    ("passo 2 volta a prometer que nome e WhatsApp bastam", SHARED,
