...
MUTACOES = [
    ...
    ("Editar plano: destaque volta a truncar", P / "admin/AdminPlanoEditor.tsx",
     '<span className="min-w-0 flex-1 break-words">{f}</span>',
     '<span className="flex-1 truncate">{f}</span>'),
    ("Editar plano: destaque perde o min-w-0", P / "admin/AdminPlanoEditor.tsx",
     '<span className="min-w-0 flex-1 break-words">{f}</span>',
     '<span className="flex-1 break-words">{f}</span>'),
    ("Editar plano: grid volta pras frações", P / "admin/AdminPlanoEditor.tsx",
     "lg:grid-cols-[300px_minmax(0,1fr)_330px]",
     "lg:grid-cols-[1fr_1.15fr_0.95fr]"),
    ("Editar plano: coluna do meio sem minmax", P / "admin/AdminPlanoEditor.tsx",
     "lg:grid-cols-[300px_minmax(0,1fr)_330px]",
     "lg:grid-cols-[300px_1fr_330px]"),
    ("Editar plano: limites voltam a 2 colunas fixas", P / "admin/AdminPlanoEditor.tsx",
     'className="grid grid-cols-1 gap-3 [&>*]:min-w-0 sm:grid-cols-2 lg:grid-cols-3"',
     'className="grid grid-cols-2 gap-3 sm:grid-cols-3"'),
    ("Editar plano: células do limite sem min-w-0", P / "admin/AdminPlanoEditor.tsx",
     'className="grid grid-cols-1 gap-3 [&>*]:min-w-0 sm:grid-cols-2 lg:grid-cols-3"',
